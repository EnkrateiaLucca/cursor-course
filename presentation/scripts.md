...
|---------|----------|-------------|
| Agent panel | Cmd+I / Ctrl+I | Primary AI workspace; cycle modes with Shift+Tab |
| Agent mode | (mode picker) | Multi-file changes, features, terminal, fixes |
| Ask mode | (mode picker) | Explore / Q&A — read-only, no edits |
| Plan mode | (mode picker) | Approve an approach before Agent builds |
| Debug mode | (mode picker) | Bugs that need runtime evidence |
| Inline Edit | Cmd+K / Ctrl+K | Quick in-place modifications |
| Tab | Tab | Accept AI predictions while typing |

Docs: [Agent](https://cursor.com/help/ai-features/agent) · [Inline Edit](https://cursor.com/help/ai-features/inline-edit)
...
2. Generate with Agent (or Ask then Agent) → show Apply / diff
3. Enhance with Inline Edit (Cmd+K) → gradient text
4. Show Tab autocomplete predictions
5. Optionally use Plan mode, then Agent → dark mode toggle
...
> Merged into Section 01 from former standalone §03.

**[Section folder →](../sections/01-cursor-intro/project-rules/)**

### Slide 12: Four Rule Types (current docs)

| Type | When Active | Example |
|------|-------------|---------|
| Always Apply | Every Agent/Ask/Plan/Debug chat | Core coding standards |
| Apply to Specific Files | Glob match in context | API validation for `**/api/**` |
| Apply Intelligently | Agent decides from description | Database / Supabase patterns |
| Apply Manually | Explicit @-mention | Performance audit checklist |

Docs: [Rules](https://cursor.com/docs/rules) — rules do **not** apply to Tab or Inline Edit.

### Slide 13: Demo — Create Rules

**→ Follow [demo-rules-setup.md](../sections/01-cursor-intro/project-rules/demo-rules-setup.md)**
...
- Show examples from [example-agents-md.md](../sections/01-cursor-intro/project-rules/example-agents-md.md) and [example-claude-md.md](../sections/01-cursor-intro/project-rules/example-claude-md.md)
...
> Merged into Section 01 from former standalone §04.

**[Section folder →](../sections/01-cursor-intro/mcp-and-tooling/)**
...
- Show config examples from [mcp-config-examples/](../sections/01-cursor-intro/mcp-and-tooling/mcp-config-examples/)

### Slide 21: CLI vs MCP

**→ Reference [cli-vs-mcp-comparison.md](../sections/01-cursor-intro/mcp-and-tooling/cli-vs-mcp-comparison.md)**
...
## Section 03: AI Quiz App Build (Slides 27–37, ~60 min)
...
**→ Use prompts from [prompts.md](../sections/03-ai-quiz-app/prompts.md) for each step**
...
**→ Reference [env-vars-reference.md](../sections/06-backend-auth-payments/env-vars-reference.md) for all env vars**
...
**[Section folder →](../sections/08-finishing/)**

### Recap

Quick walk through all 8 sections (1 sentence each):
1. **Cursor Intro** — Agent modes, Inline Edit, Tab, Project Rules, and MCP
2. **Explore-Plan-Build** — The framework that makes AI-assisted dev reliable
3. **AI Quiz App** — Context file drives the build, two-stage workflow
4. **Testing** — TDD with AI: you define behavior, AI implements
5. **Diagrams & Design** — Text-to-diagram, screenshot-to-code
6. **Backend/Auth/Payments** — Full stack with Supabase, Clerk, Stripe
7. **Deployment** — Zero-config Vercel, free tier is generous
8. **Finishing** — Resources and next steps
...
**→ Reference [resources.md](../sections/08-finishing/resources.md)**
